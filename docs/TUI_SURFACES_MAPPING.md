...
| Tech Tree | Empire menu → Tech Tree | *(no Pascal equivalent)* | **Done** (`TechTreeWindow.cs`, `Core.Entities.TechTreeReport`) — a UI real Pascal never had: tech advancement there is a per-turn random roll weighted by capital efficiency and university/ruins worlds (`UPDATE.PAS: NewTechLevel`), not a player-directed research queue, so there was nothing to browse. Every `TechCatalog` entry (`TechCatalog.AllEntries`, a new unfiltered view alongside the existing `MissingTechAt`/`UnlockedBeyond`), grouped by tech level ascending, each level's header marked when it's the viewer's own current level, each item checked off if owned. Confirmed live, not assumed: `EmpireFactory.SeedTechnology` only ever seeds `TechDev[Pred(Tech)]` (the *previous* level's full set), so items at the empire's own current level are genuinely not all owned yet — this screen is exactly "what's left to research at or above your level," the closest thing to an answer the original game's own random-roll model allows |
...
| Names Window | F9 | `NMSWIND.PAS` | **Done** (`NamesWindow.cs`, `Core.Entities.NamesWindowReport`) — single-pane, no separate column header (real Pascal's own window has none, just a bare name/location pair narrow enough for `Title` alone). Rows are every object the viewer has named, fleets first then every other kind (`InitNamesDataArray`'s own two-pass filter), non-fleet kinds ordered by `Galaxy`'s own list order per kind since this port tracks no "when was it named" history the way Pascal's persistent per-empire linked list does — also why a destroyed fleet's name just disappears here instead of surviving as Pascal's own "(destroyed)" row. Second column is `CloseUpWindow.DescribeLocation` (capital-relative coordinate, or "Fleet" for any fleet — this port's existing placeholder, since Pascal's own per-slot fleet numbering isn't modeled). Same Up/Down-marker-plus-Enter-to-Close-Up addition as the other three, plus Pascal's own "No names have been defined." empty-state message |
