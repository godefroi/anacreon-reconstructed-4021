...
| ISSP editor | Worlds menu → ISSP | `DESIGN.PAS: ChangeISSPCom` | **Done** (`IsspEditor`) — 4-row stepper over `SelfSufficiencySettings` (Chemical/Mining/Supply/Trillum), Left/Right clamped-step through the 11 named bands, Up/Down/PageUp/PageDown wraps the row cursor, Esc/Enter both exit (edits are already live, same no-cancel convention as Fleet Group Configuration/Defenses). Planet-only — a starbase's dial is hardcoded at 0 in real Pascal too |
