...
| Tactical Battle Display | During an engagement | `ATTCOMM.PAS: DrawScreen`/`Engage`/`GroupMove`/etc. | **Done** (`TacticalBattleDisplayWindow.cs`, round driver in `Core.Combat.InteractiveCombat`) — genuinely full-screen `View`, `Application.AddTimeout` for the message-line auto-clear; decorative art (`DrawObject`/`DrawGrid`/`DrawStars`) not reproduced (docs/OPEN_GAPS.md) |
