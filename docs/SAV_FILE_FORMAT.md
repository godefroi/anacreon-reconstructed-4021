...
| `STARGATE_PREP.SAV` | (hand-built from `INTRO_1.SAV`, not real play) | max tech + a supply fleet at the capital, purpose-built so the player could start building a stargate |
| `STARGATE_STARTED.SAV` / `STARGATE_NEARDONE.SAV` / `STARGATE_DONE.SAV` | INTRO, played forward from `STARGATE_PREP.SAV` | a populated `ConstrRecord` mid-build, then a completed `StargateRecord` — the last previously-unconfirmed section |
...
**Empirically confirmed**: `STARGATE_DONE.SAV` has one completed stargate (index 50) at `(8,18)`
— `GTyp = 24` (`gte`, at its absolute `TechnologyTypes` ordinal, same convention as
`ConstrRecord.CTyp`), `Emp = 0`, `Dest = (0,0)` (`Limbo`, `GALAXY.PAS:49` — a lone gate has no
linked destination yet), `NextID = EmptyQuadrant`. Both match `INTRFACE.PAS:410-427`
(`CreateStargate`) exactly, including that it resets `ScoutedBy`/`KnownBy` to just `[Emp]` even
though three other empires had scouted the *construction site* beforehand (see below) — a brand
new object starts with a clean scout history.

This save is the end of a hand-built progression that also closed out the last open question
about `ConstrRecord` completion. Starting from `STARGATE_PREP.SAV` — a synthetically-edited save
(not from real play) that bumped the player empire's tech to `GteTchLvl` and parked six fleets
carrying the full 15-year `gte` material requirement next to the capital, per
`AnacreonManual.md`'s Construction Requirement Table and `DATACNST.PAS:528-548`
(`YearsToBuild`/`ConsCargoNeeded`) — the user actually played the game forward and saved three
more times:

| File | Year | `Constructions` | `Stargates` | `Sector[8,18].Obj` |
|---|---|---|---|---|
| `STARGATE_STARTED.SAV` | 4021 | `CTyp=24`, `TimeToCompletion=15` | none | `{Con, 50}` |
| `STARGATE_NEARDONE.SAV` | 4034 | `CTyp=24`, `TimeToCompletion=2` | none | `{Con, 50}` |
| `STARGATE_DONE.SAV` | 4036 | *(removed)* | `GTyp=24` at `(8,18)` | `{Gate, 50}` |

`TimeToCompletion` counts down by exactly 1 per year (15 at year 4021, 2 at year 4034 — 13 years
elapsed, 13 decrements) with no delays, confirming the fleets' cargo was sufficient every single
year (`UPDATE.PAS:103-220`, `UpdateConstruction`/`UseUpRawMaterial`). Completion two years later
(year 4036) removes the `ConstrRecord` entirely and adds a `StargateRecord` at the same index
(50) — both `MaxNoOfConstrSites` and `MaxNoOfStargates` are 50, and slot allocation in both
`Construction` (`INTRFACE.PAS:486-492`) and `NextStargateSlot` (`INTRFACE.PAS:399-408`) starts
from the top and counts down, so with no other constructions or gates active, both happened to
land on slot 50 — coincidence of allocation strategy, not a shared index space. `Sector[8,18].Obj`
tracks the site through both phases: `{Con, 50}` while under construction, replaced with
`{Gate, 50}` on completion (confirms the empirically-derived `ObjectTypes` ordinals `Con=1` and
`Gate=4`, see [ObjectTypes](#objecttypes)).
...
was empty — a fresh scenario start, saved before any turn was played. Twelve more real saves,
generated by actually starting different scenarios and playing turns (plus one hand-built starting
point for the stargate progression, played forward from there), closed every remaining gap:
...
- **`FLEET_ORDERS.SAV`** (hand-edited from an existing save): a fleet with a real, non-empty
  ...
  the full decode.
- **`STARGATE_PREP.SAV`** (hand-built, not real play) **→ `STARGATE_STARTED.SAV` →
  `STARGATE_NEARDONE.SAV` → `STARGATE_DONE.SAV`** (played forward from there): the last remaining
  gap. `STARGATE_PREP.SAV` bumped the player empire's tech to `GteTchLvl` and parked six fleets
  with the full 15-year `gte` material requirement (per `AnacreonManual.md`'s Construction
  Requirement Table, cross-checked against `DATACNST.PAS:528-548`) next to the capital, sized so
  each fleet's transport count actually covers its cargo (`MISC.PAS:202-222`, `FleetCargoSpace`)
  — then the user played it forward and saved three more times, capturing a `ConstrRecord`
  mid-build (`TimeToCompletion` counting down 15 → 2 with no delays) and finally a completed
  `StargateRecord`. See [Stargates](#stargates) for the full decode.
...
Every record type in this document has now been independently confirmed against at least one
real save, not merely derived from the Pascal declarations — including `StargateRecord`, the last
holdout, closed by the `STARGATE_PREP`/`STARTED`/`NEARDONE`/`DONE` progression above. One caveat:
`STARGATE_PREP.SAV` itself was hand-built (tech level and fleet placement edited directly, not
reached through normal play) to make the 15-year gate build reachable in a reasonable number of
turns; everything downstream of it (`STARTED`, `NEARDONE`, `DONE`) is the real game engine's
response to that starting state, saved after actually playing the turns forward.
...
the actual DOS game and cross-checking field values on screen. All thirteen reference saves pass
in both scripts.
