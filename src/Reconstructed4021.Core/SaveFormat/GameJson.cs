...
public static class GameJson
{
    ...
    /// <see cref="EmpireRefConverter"/> instead of full inline serialization, plus
    /// <see cref="Fleet"/>'s own <c>Orders</c> property (its <see cref="FleetOrder.DestinationObject"/>
    /// has the exact same forward-reference problem <c>Owner</c> does, only worse -- see
    /// <see cref="FleetOrderListConverter"/>'s own doc comment for why its read side is a no-op)
    /// through <see cref="FleetOrderListConverter"/>. Built fresh per call (not cached) since both
    /// converters close over this call's <paramref name="index"/>.
    /// </summary>
